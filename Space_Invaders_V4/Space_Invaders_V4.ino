...
#define PIN_BTN_RED 10
#define PIN_BTN_GREEN 7
#define PIN_BTN_RESET 4
#define PIN_BTN_CONTINUE 5  // <<< NEW Continue button- go to the next level
...
#define PIN_STATUS_LED 19              //  Status LED that follows the sacrificial‑LED blink
static unsigned long lastBlink19 = 0;  // timestamp of the last toggle
static bool statusState = false;       // current level of status (HIGH/LOW)
...
void loop() {
  ...
  /* --------------------------------------------------------------
     Reset button (GPIO 32) – Reset / WiFi setup
     -------------------------------------------------------------- */
  if (digitalRead(PIN_BTN_RESET) == LOW) {  // button pressed
    if (!btnResetHeld) {                    // first edge
      btnResetHeld = true;
      btnResetPressTime = now;
    } else {
      // Long‑press (>3 s) → Wi‑Fi configuration mode (unchanged)
      if (now - btnResetPressTime > 3000) {
        FastLED.clear();
        if ((now / 250) % 2 == 0) {
    ...
    if (btnResetHeld) {  // we just let go
      unsigned long pressLen = now - btnResetPressTime;
      btnResetHeld = false;
  ...
  if (levelJustFinished) {
    ...
    if (now - lastBlink >= 250) {        // 250 ms → 2 Hz
      ...
      statusState = !statusState;  // invert state
      digitalWrite(PIN_STATUS_LED,
                   statusState ? HIGH : LOW);  // HIGH = LED ON
    }
  }
  ...
  if (currentState == STATE_MENU) {
    ...
    if (digitalRead(PIN_BTN_RESET) == LOW && !btnResetHeld) {
      btnResetHeld = true;
      btnResetPressTime = now;
    }
    if (btnResetHeld && digitalRead(PIN_BTN_RESET) == HIGH) {
      btnResetHeld = false;
  ...
  if (currentState == STATE_PLAYING || currentState == STATE_BOSS_PLAYING) {
    ...
    // --------------------------------------------------------------
    // make sure status is off when the level‑finished flag is cleared
    // --------------------------------------------------------------
    if (!levelJustFinished && statusState) {
      statusState = false;
      digitalWrite(PIN_STATUS_LED, LOW);
    }
